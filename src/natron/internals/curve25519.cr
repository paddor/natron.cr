  ...
  module Internals
    module Curve25519
      ...
      def self.scalarmult(scalar : Bytes, u : Bytes) : Bytes
        ...
        buf = Bytes.new(BYTES)
        Sodium.check(
          LibSodium.crypto_scalarmult_curve25519(buf.to_unsafe, scalar.to_unsafe, u.to_unsafe),
          "scalarmult"
        )
        buf
      end
      ...
      def self.scalarmult_base(scalar : Bytes) : Bytes
        ...
        buf = Bytes.new(BYTES)
        Sodium.check(
          LibSodium.crypto_scalarmult_curve25519_base(buf.to_unsafe, scalar.to_unsafe),
          "scalarmult_base"
        )
        buf
      end
